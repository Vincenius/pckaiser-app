...
- Updates must never break running games (`game_core/src/state/versioning.dart`): `GameState` JSON changes must be additive (new field + `fromJson` default); incompatible reshapes bump `currentSchemaVersion` plus a `schemaMigrations` step. Gameplay rule/balance changes bump `currentRulesVersion` and gate on `state.rulesVersion >= n` — but **every game plays the latest rules**: saves adopt them on load (`adoptLatestRules`, applied in the client's `SaveService.load`); the gates exist for documentation/testability, not to pin running games. See ARCHITECTURE.md "Versioning & compatibility".
