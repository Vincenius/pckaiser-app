...
- Updates never break running games (`game_core/src/state/versioning.dart`): JSON changes additive (new field + `fromJson` default); reshapes bump `currentSchemaVersion` + migration. Gameplay changes bump `currentRulesVersion` and gate on `state.rulesVersion >= n` — but **every game plays the latest rules** (saves adopt them on load via `adoptLatestRules`); gates are temporary documentation, consolidated into the baseline at each release (release 1 = ruleset v1). See ARCHITECTURE.md "Versioning & compatibility".
