...
```sql
players       (id UUID PK, display_name TEXT, fcm_token TEXT,
               push_opt_out TEXT[],     -- optional notification kinds switched off
               created_at)
...
**Double clock for the opening move (2026-08-24, user request).** A side that has not yet acted in this war (`war.actedSlots`) gets **twice** `settings.war_round_timeout` for that one round; every later round runs on the normal clock. It is exactly the round the no-show rule keys on, and the one players kept missing: the duel starts at an appointment made hours earlier, not off the player's own last move, so the reaction window has to cover "notice that the war has begun" rather than "make your next move". The grace is granted at most once per side, so a live duel keeps its pace (`_commit`'s war-round branch).

**Reclaiming a delegated side mid-war (2026-08-24, user request).** `ResumeWarCommand` reverses exactly the no-show handover, at any point, for the side that submits it — the one direction `WarPrepPlan` deliberately does not cover once the rounds are running. Accepted OUT OF TURN like `WarPrepPlan`/`SetTroopStance` (`war.autoSlots.contains(slot)` is enough proof this side is never the awaited player); it simply removes the slot from `war.autoSlots`, so `warSideIsHuman`/`warActingSlot` pick it up again from the very next check — including the round already under way, since `war.actingSlot` itself was never touched by the delegation. The client surfaces it in the off-turn map viewer's `WarPanel` (which now also mounts during the ROUNDS phase, not only preparation) as a "Kontrolle übernehmen" button whenever the seat's own side sits in `autoSlots`.
...
**Per-player notification settings (2026-08-24, user request).** Every notification kind is a `PushKind` string (`backend/lib/src/models.dart`, mirrored by `client/lib/services/push_kinds.dart` — the wire contract). `PushKind.optional` — `war_started`, `war_start_fixed`, `war_start_soon` — is switchable under **Options ▸ Benachrichtigungen**, all on by default; everything else (`your_turn`, `your_decision`, `match_expiring`) is essential and can never be muted, so nobody loses a turn or a match to a setting. The choice is stored as an **opt-OUT list** (a kind added by a later build therefore starts out on, exactly as on a fresh install): locally in `SettingsService.pushOptOut` for the screen, and authoritatively on the player record (`players.push_opt_out`), uploaded by `OnlineService.syncPushPrefs` on every change and again on launch with the token. The sending side reads it in ONE place — `MatchService._pushTarget(playerId, kind)` returns null for a muted kind, so no call site can bypass it — and `updatePlayer` drops anything outside `PushKind.optional` off the wire. A rename (`registerPlayer` upsert) carries the set over, like the token.
