...
| Colony ship steered tile-by-tile over water (1 movement point per water tile) | **Rules v6:** `SendShip` — flat 1 movement point + 700 T, target must be water-reachable from an own Hafen. Two UI entry points: tap the own Hafen → "Schiff aussenden" → pick the free land target (tile-pick, invalid picks explain why), or tap a sea-reachable free land tile directly | Tile-by-tile steering doesn't fit touch UX; the sea-route requirement is kept (BFS), only the per-tile cost is collapsed into one Zug |
...
| Drilling ("Truppe ausbilden") unbounded, no per-turn limit traced | **Rules v7:** `DrillTroop` +1 quality for 5 T/man (the traced original), capped at quality 10. **Rules v8:** the v7 once-per-unit-per-turn limit is lifted (user decision) — the quality cap alone bounds total drilling | The quality cap keeps total drill spend bounded; the per-turn limit only stretched it over rounds and was dropped as needless friction |

Rows marked **Rules v2/v3/v4/v5/v6/v7/v8** are gated on `state.rulesVersion >= n`
(see `game_core/src/state/versioning.dart`). Since 2026-06-11 every game
plays under the **latest** ruleset: saves adopt new rules on load
(`adoptLatestRules` in `SaveService.load`), so the gates exist to
document each change and keep old behavior testable — not to pin
running games.
