...
| Combat: casualties scale with the raw strength ratio | Casualty superiority = √(men ratio) × per-man-power ratio (2026-07-14): mass is dampened (~18× men for a guaranteed rout, was ~4.3×), drilled quality and class count in full | Bigger armies did disproportionate damage to smaller ones; training barely paid off (user report) |
| Troop classes differ only in raw power (§10.1) | Tactical class roles (2026-07-14): Infanterie +1 defense on defended tiles, Kavallerie ×1.2 in open-field clashes, Artillerie halves the enemy's tile defense | Each class gets a job (walls / field battle / siege) beyond the power table |
