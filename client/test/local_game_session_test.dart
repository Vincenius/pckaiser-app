...
void main() {
  ...
  test('create advances past leading AI realms to the first human',
      () async {
    ...
    final session = await LocalGameSession.create(
      ...
      setup: GameSetup(
        humans: [
          HumanPlayerSetup(
              founderName: 'Otto', gender: 0, countrySlot: 5, dorfName: 'Ulm'),
        ],
        reformationYear: 1020,
        ottomanYear: 1040,
        seed: 7,
    ...
    expect(session.state.currentPlayer, 5,
  ...
  test('resume heals saves parked on an AI slot (pre-fix saves)', () async {
    final session = await LocalGameSession.create(
        slotName: 'Alt', setup: setup(), saves: saves);
    ...
    final resumed =
        await LocalGameSession.resume(slotName: 'Alt', saves: saves);
    expect(resumed.state.currentPlayer, 1,
        reason: 'resume must advance past AI slots to the human');
    expect((await saves.load('Alt')).currentPlayer, 1,
  ...
  test('resume continues from the saved state', () async {
    ...
    expect(session.state.currentPlayer, 1);
    expect(session.state.year, greaterThan(1000),
